...
(function () {
  ...
  function pickFrame(total, lowS, hiS) {
    if (lowS >= 0.52) {           /* rounded, low-frequency vowels -> O cells */
      return total > 0.40 ? 6 : 3;
    }
    if (hiS >= 0.40) {            /* sibilants and fricatives */
      return total > 0.34 ? 4 : 7;
    }
    if (total > 0.42) { return 4; }
    if (total > 0.28) { return 1; }
    return 5;
  }
  ...
  function speechEnd() {
    ...
    setFrame(2);   /* friendly landing grin */
