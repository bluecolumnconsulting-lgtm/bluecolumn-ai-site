/* ===============================================================
   OutLoud for AUTOMATTIC — RUNTIME BOOTSTRAP (sprite edition)
   Cloned from outloud-v2/runtime.js. Wires the layers in spec order:
     realtime input → session orchestrator →
     knowledge (business-knowledge) + memory (customer-memory) →
     reasoning (response-planner) → rendering
       (speech-director + avatar-director + content-director)

   Avatar: the orange mascot only (sprite viseme rig, lip-sync via
   Web Audio analyser). No Simli video session on this page — so
   there is no video handshake and no video fallback copy.
...
(function () {
  ...
  /* Sprite edition: no Simli sink — speech blobs play through
     <audio> and the avatar's analyser drives the visemes. */
  ...
  /* ---------- Greeting gate (voice needs a gesture) ----------
     Browsers block autoplay audio — the gate requires a real
     gesture. On tap: the greeting runs through the mascot sprite
     and the ElevenLabs voice. If the voice cannot start, the
     transcript still carries the reply. */
  var gateEl = null;
  function removeGate() {
  ...
  /* Greeting runs through the full pipeline as a validated plan turn
     ...
     lands on the transcript via the speech chunk walk. With voice
     output, the greeting waits for the tap gate: a gesture is
     required before any audio may play. */
  showGate();
  /* Typed input (or mic) before tapping the gate still works — a
     real click IS a valid user gesture, so audio is allowed here. */
